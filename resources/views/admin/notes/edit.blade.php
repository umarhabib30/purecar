    ...
    <div class="row">
        <div class="col-12">
            <div class="card shadow-lg mb-4 border-0">
                <div class="card-header bg-warning text-dark p-4 rounded-top">
                    <h2 class="text-dark text-capitalize mb-0">Edit Note ✏️</h2>
                </div>
                ...
                <div class="card-body p-4 bg-light">
                    <form action="{{ route('admin.notes.update', $note) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6 border-end">
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="content" id="content" class="form-control form-control-lg" rows="5" required placeholder="Enter a descriptive note">{{ old('content', $note->content) }}</textarea>
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="type" id="type" class="form-select form-select-lg" required>
                                        <option value="success" class="text-success" {{ $note->type == 'success' ? 'selected' : '' }}>✅ Success (e.g., Approved)</option>
                                        <option value="warning" class="text-warning" {{ $note->type == 'warning' ? 'selected' : '' }}>⚠️ Warning (e.g., Needs review)</option>
                                        <option value="danger" class="text-danger" {{ $note->type == 'danger' ? 'selected' : '' }}>🛑 Danger (e.g., Blocked)</option>
                                    </select>
                                </div>

                                <div class="alert alert-info">
                                    <strong>Created by:</strong> {{ $note->creator->name ?? 'N/A' }}<br>
                                    <strong>Created at:</strong> {{ $note->created_at->format('M d, Y h:i A') }}
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 ps-md-5">
                                ...
                                <div class="mb-4">
                                    <label for="selected_adverts" class="form-label fw-bold text-dark">
                                        Select Adverts (Max 100 results)
                                        <span class="badge bg-secondary">{{ count($selectedAdvertIds) }} currently selected</span>
                                    </label>
                                    <select name="selected_adverts[]" id="selected_adverts" class="form-select" multiple size="10" required>
                                        @foreach($adverts as $advert)
                                            <option value="{{ $advert->advert_id }}" {{ in_array($advert->advert_id, $selectedAdvertIds) ? 'selected' : '' }}>
                        ...
                        <div class="d-flex justify-content-end pt-4 border-top mt-4">
                            ...
                            <button type="submit" class="btn btn-warning btn-lg shadow-sm text-dark">
...
$(document).ready(function() {
    // Store initially selected adverts
    var initiallySelected = @json($selectedAdvertIds);
    ...
    $('#apply_advert_filter').on('click', function() {
        ...
        var currentlySelected = selectElement.val() || [];
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    $.each(data, function(index, advert) {
                        var isSelected = currentlySelected.includes(advert.id.toString());
                        var option = $('<option></option>')
                            .attr('value', advert.id)
                            .text(advert.text);
