<style>
 ...
 <script>
/**
 * Car search form script (with mobile Back-button closing the full-screen select modal)
 * Safe behavior:
 * - Only on mobile (<= 768px)
 * - Only when a .custom-select-modal is open
 * - Cleans up pushed history entries on normal close
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    // -------------------------
    // Mobile back button closes open select modal
    // -------------------------
    let __pcIgnoreNextPopstate = false;

    function anyCustomSelectModalOpen() {
        // Works even if modal was moved to <body>
        return !!document.querySelector('.custom-select-modal[style*="display: flex"]');
    }

    function pushSelectModalHistory(modal) {
        if (!isMobile() || !modal || !window.history || !history.pushState) return;
        if (modal.dataset && modal.dataset.historyPushed === '1') return;

        history.pushState({ __pcSelectModal: true }, '', window.location.href);
        if (modal.dataset) modal.dataset.historyPushed = '1';
    }

    function cleanupSelectModalHistoryIfNeeded(modal) {
        if (!isMobile() || !modal || !window.history || !history.back) return;
        if (!(modal.dataset && modal.dataset.historyPushed === '1')) return;

        __pcIgnoreNextPopstate = true;
        history.back(); // removes the pushed state
        if (modal.dataset) modal.dataset.historyPushed = '';
    }

    window.addEventListener('popstate', function () {
        if (__pcIgnoreNextPopstate) {
            __pcIgnoreNextPopstate = false;
            return;
        }
        if (!isMobile()) return;

        if (anyCustomSelectModalOpen()) {
    ...
    function applyDropdownItemSelection(item, forceForm) {
        ...
        if (modal) {
            ...
            cleanupSelectModalHistoryIfNeeded(modal);
        }
    }
    ...
    function closeAllCustomSelectModals() {
        document.querySelectorAll('.custom-select-modal').forEach(function (m) {
            ...
            if (wasOpen) cleanupSelectModalHistoryIfNeeded(m);
        });
        setBodyScrollLocked(false);
    }
    ...
    document.querySelectorAll('form.filter-box').forEach(function (form) {
        ...
        // Initial: expand if URL indicates advanced=1
        (function () {
            const qs = window.location && window.location.search ? window.location.search : '';
            const params = new URLSearchParams(qs);
        ...
        // Initial load:
        // - If server provided initial facets, render immediately
        // - Else fetch facets/options from /filter
